...
(function(window, undefined) {
  ...
  EventEmitter.prototype.removeEventListener = function ee_rm(type, listener) {
    ...
    var position = typeListeners.indexOf(listener);
    if (position === -1) {
      return;
    }

    typeListeners.splice(position, 1);
  };
  ...
  function parseEntity(id, value, ast) {
    ...
    if (position !== -1) {
      name = id.substr(0, position);
      key = id.substring(position + 1, id.length - 1);
    } else {
      name = id;
      key = null;
    }
  ...
  function loadINI(url, callback) {
    ...
    io.load(url, function(err, source) {
      var position = ctx.resLinks.indexOf(url);

      if (err) {
        // remove the ini link from resLinks
        ctx.resLinks.splice(position, 1);
        return callback(err);
      }
      ...
      ctx.resLinks.splice.apply(ctx.resLinks, [position, 1].concat(patterns));
      callback();
    });
  }
  ...
  function translateFragment(element) {
    if (!element) {
      ...
      document.documentElement.direction = this.language.direction;
    }
  ...
  function translateElement(element) {
    ...
    for (var key in entity.attributes) {
      if (entity.attributes.hasOwnProperty(key)) {
        ...
        if (position !== -1) {
          element[key.substr(0, position)][key.substr(position + 1)] = attr;
